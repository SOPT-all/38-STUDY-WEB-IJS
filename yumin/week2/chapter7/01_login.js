...
const updatePupilPositionByMouse = (mouseEvent, $eye) => {
  ...
  const distance = Math.min(
    $eye.offsetWidth / options.scaleFactor,
    calculateDistance(mouseX, mouseY, eyeCenterX, eyeCenterY),
  );
...
document.addEventListener("DOMContentLoaded", () => {
  const isPasswordType = (e) => "password" === e.target.id;
  const $character = document.querySelector(".character");
  const toggleCloseClass = (condition) =>
    $character.classList.toggle("close", condition);
  document.querySelectorAll("input").forEach(($input) => {
    ...
    $input.addEventListener("input", (e) => {
      ...
      isPasswordType(e)
        ? $character.classList.add("close")
        : updatePupilPositionByKeyboard(e);
    });
  });
...
document.addEventListener("mousemove", (e) => {
  ...
  document
    ...
    .forEach(($eye) => updatePupilPositionByMouse(e, $eye));
});
